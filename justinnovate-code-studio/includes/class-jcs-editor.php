...
class JCS_Editor {
	...
	public function register_hidden_page() {
		add_submenu_page( null, __( 'Builder Canvas', 'jcs' ), __( 'Builder Canvas', 'jcs' ), 'edit_posts', 'jcs-editor', array( $this, 'render_editor' ) );
	}

	public function get_editor_url( $post_id, $lang = 'en' ) {
		return add_query_arg( array( 'page' => 'jcs-editor', 'post' => $post_id, 'lang' => $lang ), admin_url( 'admin.php' ) );
	}

	public function maybe_render_editor( $screen ) {
		if ( ! isset( $screen->id ) || false === strpos( $screen->id, 'jcs-editor' ) ) return;
		$this->render_editor();
		exit;
	}
	...
	public function render_editor( $forced_post_id = 0, $frontend = false ) {
		...
		if ( ! $post || JCS_CPT::POST_TYPE !== $post->post_type ) wp_die( esc_html__( 'Banner not found.', 'jcs' ) );
		if ( ! current_user_can( 'edit_post', $post_id ) ) wp_die( esc_html__( 'You do not have permission to edit this.', 'jcs' ) );
		...
		wp_localize_script( 'jcs-editor', 'JCS_EDITOR_DATA', array(
			'postId' => $post_id,
